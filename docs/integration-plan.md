...
2. **Tick service**: the game-scripts page documents a static `game.tickRateHz` declaration — fractional rates down to `0.01`, only an explicit `0` disables ticks, and a missing or invalid declaration takes the platform's 0.25 Hz default. **Poker declares `tickRateHz: 1`** in `server.js`: one-second sweeps make idle-timeout enforcement granularity ≤ 1 s (no operator request needed). The client paces outgoing commands to the same cadence (`GAME.cmdMinIntervalMs`, queued in `game-socket.js`) so it never sends faster than the tick rate. That pacing is our own discipline, not a platform limit: `ws/v1/games` has no per-sender rate limit and will not close the socket for exceeding one (the tick-derived `PolicyViolation` drop applies to `ws/v1/relay` and `ws/v1/realtime`, which poker does not use for gameplay). AI seats are additionally driven at the tail of every mutating path (human action, timeout chain, tick), so the table never idles on an AI turn between sweeps, and error responses after a sweep still return `sessionState` + broadcasts so the sweep is never rolled back.
